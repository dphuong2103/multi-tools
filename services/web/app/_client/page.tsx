...
export default function ClientPage() {
    ...
    async function testGet() {
        try {
            var res = await authAxios.post("http://localhost:8080/api/v1/books",JSON.stringify({
                name: bookName
            }))
    ...
    async function testPost() {
        try {
            try {
                var res = await authAxios.put("http://localhost:8080/api/v1/books",JSON.stringify({
                    id: 1,
                    name: bookName
                }))
                console.log("Res: ", res.data);
